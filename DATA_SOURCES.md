...
- **[Sleeper API](https://docs.sleeper.com/)** — player master list, ADP,
  trending adds/drops, projections. Their **weekly projection snapshot**
  (`sleeper-weekly-proj-*.json`, the external benchmark in the weekly
  backtest) is **LOCAL-ONLY and never committed** — it is exactly the kind of
  bulk dump the tier above says not to redistribute. Only StatHead's derived
  accuracy metrics ship. Regenerate it under your own access with
  `python3 scripts/fetch-sleeper-weekly-projections.py`.
...
- **In-season projections & backtest** — `inseason-projections-*.json`,
  `inseason-params.json`, `weekly-backtest-*.json`. Model outputs and accuracy
  metrics. The backtest *compares against* Sleeper but contains only aggregate
  error statistics, not their numbers.
