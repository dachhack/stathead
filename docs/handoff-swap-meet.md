...
| Swap Meet page (Dynasty → Swap Meet, tab `swap-meet`; deep link `?tab=swap-meet` or `#/swap-meet`, `parsePageRoute` / `pageUrl` / `clearPageRoute` in `src/lib/hashRoute.ts`, read in `App.tsx` for the initial tab and on `hashchange`, then stripped from the address bar) | `src/components/SwapMeetHome.tsx` — brand header with "Copy a link to this page", the meets this device opened or joined (`listMeets`, copy links, forget), "open a meet from a link" (paste a share link, hash link or id), and "Start a meet" two ways: **From a league** (`TradeFinisher` in `mode="swap"`: open by default, no calculator hand-off, the composer at the end; Sleeper or ESPN) or **By hand, no league** (`src/components/swap/ManualMeetBuilder.tsx`: name both sides, format + TE premium, pick pieces off the dynasty board with `BoardPicker`, a pitch, create → two links; the meet's `league.source` is `'manual'` and its two "teams" hold only the pieces on the table). Loads the dynasty board itself via `fetchDynastyRankingsForDisplay(format)`; the finisher's `onLeagueDetected` switches format / TE premium to the league's. The finisher's selections live in sessionStorage, so a league picked in the Trade Calculator carries over |
| Composer (end of the finisher in swap mode) | `src/components/swap/SwapMeetComposer.tsx` — pick candidates, per-candidate pitch drafted from `partnerPositives`, create → two links. Every version's pieces can be changed ("Change the pieces" → `SideEdit` per side: the pieces with a × each, a search over that team's roster to add one; the finisher's throw-ins are suggestions), a version can be added by hand ("+ A version by hand", starts from the offer as built), and each change is re-priced through the finisher's `revise` prop (`evaluateOffer` + a fresh pitch; the label gets "· changed", "Back to the finisher's" restores). Edits live in the composer (`edits` by candidate key, `custom` list) and reset when the finisher's candidates change. A version with an empty side blocks Create with a message. The Trade Calculator's finisher (`mode="calculator"`) no longer embeds it: it shows a "Send to <partner> · Swap Meet →" hand-off (`onOpenSwapMeet`, wired in `App.tsx` to the tab) |
...
`npx vite --port 5179 --host 127.0.0.1` (127.0.0.1 so the local snapshots
serve; the app is at `/stathead/`), stub the worker in-page with an
...
in three browser contexts (desktop, partner, mobile). For the finisher +
composer, curl the league's `/league/<id>`, `/rosters`, `/users`,
`/traded_picks` and `/players/nfl` from api.sleeper.app into fixtures,
`page.route('https://api.sleeper.app/**')` them, and seed
`sessionStorage['stathead:trade-finisher'] = {source:'sleeper', leagueId,
open:true}` before opening `?tab=swap-meet`: the league auto-loads and the
two roster selects appear. The note input and
