...
| Swap Meet page (Dynasty → Swap Meet, tab `swap-meet`) | `src/components/SwapMeetHome.tsx` — brand header, the meets this device opened or joined (`listMeets`, copy links, forget), "open a meet from a link" (paste a share link, hash link or id), and "Build the trade": `TradeFinisher` in `mode="swap"` (open by default, no calculator hand-off, the composer at the end). Loads the dynasty board itself via `fetchDynastyRankingsForDisplay(format)`; the finisher's `onLeagueDetected` switches format / TE premium to the league's. The finisher's selections live in sessionStorage, so a league picked in the Trade Calculator carries over |
