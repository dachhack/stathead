...
4. 🟡 **Consensus Projections** = blend Clay + our base. Clay 2026 guide now extracted: `scripts/extract_clay_projections.py <pdf>` → `public/data/clay-projections-2026.json` (448 offensive players QB/RB/WR/TE/K, 98% joined to player_key) (PR #326). ⚠️ The PDF itself is a manual drop, NOT committed (ESPN's; surfaced only as "Consensus"). Re-run the extractor on each new guide. **Still open**: the 80% Clay / 20% us blend as a "Consensus" projection setting in the engine; richer pipelines (unit grades p63 → SOS opponent-quality, projected SOS p62, team projections pp2-33, IDP, win prob).
