...
// How much to blend prior team totals with league average (regression to mean)
const TEAM_WEIGHT = 0.60;
const LEAGUE_WEIGHT = 0.40;
...
function computeVegasMultiplier(
  ...
  // Use prior-season game lines to compute team implied total vs league avg
  ...
  const blended = teamAvg * 0.6 + leagueAvg * 0.4;
  return Math.max(0.85, Math.min(1.15, blended / leagueAvg));
}
    ...
    async function run() {
      try {
        ...
        for (const season of TEST_SEASONS) {
          if (cancelled) return;
          setLoadingStatus(`Loading ${season - 1} stats (prior season for ${season} projections)...`);
          ...
          // Build comparison rows for teams that exist in both seasons
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
        Projects team totals using prior-season data (60% team + 40% league avg × Vegas multiplier),
        then compares against actual results. This validates the top-down methodology before applying it to player projections.
      </p>
      ...
      {selectedSeason !== 'all' && currentData && (
        <div style={{ marginBottom: 24 }}>
          <h4 style={{ marginBottom: 8 }}>
            {selectedSeason} Team-by-Team — Projected (from {selectedSeason - 1} data) vs Actual
