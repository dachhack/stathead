...
export async function buildAskDataSystemPrompt(): Promise<string> {
  ...
  return [
    ...
    '    WHERE s.season_type = \'REG\' GROUP BY c.name',
    '- To find a `player_key` from a name, query `player_crosswalk` with ILIKE on `display_name` or `all_names`, filter by `position`, then use the returned `player_key` for subsequent joins. Check `is_college_only` to distinguish pre-draft prospects from NFL-registered players.',
    '- Player names are messy across sources (`Ja\'Marr Chase`, `Amon-Ra St. Brown`, `Michael Penix Jr.`). If you have to match on name, use ILIKE and prefer `player_display_name` from `player_stats` or `display_name` from `player_crosswalk`.',
