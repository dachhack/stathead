/**
 * Dynasty time-series forecast utilities — shared between DynastyForecast and TradeCalculator.
 *
 * Loads pre-computed forecasts from dynasty-forecasts-{1qb,superflex}.json
 * (generated by scripts/train_dynasty_timeseries.py with full feature vectors).
 */
...
export function loadForecasts(format: '1qb' | 'superflex' = '1qb'): Promise<ForecastCache | null> {
  if (!_forecastPromises.has(format)) {
    _forecastPromises.set(format,
      fetch(`${import.meta.env.BASE_URL}data/dynasty-forecasts-${format}.json`)
...
// per-player ratio from dynasty-fc-rescale.json. logReturn is scale-invariant and
...
/** Look up a Dynasty player's redraft PPG, adjusted for TEP tier.
...
/**
 * TE Premium dynasty value multipliers derived from Dynasty's TE+/++/+++ tiers.
 * Empirical averages from Dynasty Superflex rankings (Apr 2026):
