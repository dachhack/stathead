...
# Additive bridge from the feature store's name::season keys to the crosswalk's
# stable player_key (see emit at the end of build()). Lives under feature-store/
# so it ships alongside the shards it indexes.
KEY_INDEX_OUT = ROOT / 'public/data/feature-store/key-index.json'
...
def main():
    ...
    # Capture the name::season -> player_key resolution as we go; this is the
    # source of truth for the additive feature-store key index emitted below.
    feature_key_index: dict[str, str] = {}
    ...
    if profile_path.exists() and players_path.exists():
        ...
        for key in profile:
            ...
            try:
                ...
                pass
            if pos:
                pk = resolve('stathead_adp', name, pos, year)
                if pk:
                    feature_key_index[key] = pk
    ...
    # ── Emit feature-store key index (additive #8) ──
    # Bridges the feature store's name::season shard keys to the crosswalk's
    # stable player_key, so consumers can read features by canonical identity
    # (collision-proof) without re-keying the ~17MB of shards. Built from the
    # exact resolution the crosswalk already performs on the stathead_adp
    # source, so it can never drift from the crosswalk. COL→NFL promotions are
    # folded in so keys reflect the final canonical id.
    promo_map = {p['old_player_key']: p['new_player_key']
                 for p in promotions_log
                 if p.get('old_player_key') and p.get('new_player_key')}
    by_name_key = {k: promo_map.get(v, v) for k, v in feature_key_index.items()}
    by_player_key: dict[str, str] = {}
    collisions = 0
    for name_key, pk in by_name_key.items():
        season = name_key.rsplit('::', 1)[-1]
        pk_season = f'{pk}::{season}'
        if pk_season in by_player_key:
            collisions += 1
            continue  # keep first; collisions logged in the count below
        by_player_key[pk_season] = name_key
    key_index_doc = {
        '_about': (
            'Additive bridge between the feature store name::season shard keys '
            'and the crosswalk player_key. byNameKey maps a shard key '
            '("<normalized_name>::<season>") to its stable player_key; '
            'byPlayerKey maps "<player_key>::<season>" back to the shard key so '
            'features can be looked up by canonical identity. Regenerated by '
            'build-player-crosswalk.py from the same resolution used to build '
            'the crosswalk; do not hand-edit.'
        ),
        'version': 1,
        'count': len(by_name_key),
        'byNameKey': by_name_key,
        'byPlayerKey': by_player_key,
    }
    KEY_INDEX_OUT.write_text(json.dumps(key_index_doc, separators=(',', ':')))
    print(f'Wrote {KEY_INDEX_OUT} — {len(by_name_key)} name->player_key mappings'
          + (f', {collisions} player_key::season collisions skipped' if collisions else ''))
