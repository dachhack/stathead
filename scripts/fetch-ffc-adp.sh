...
#
# Writes one file per season: public/data/ffc_adp_ppr_<year>.json
# (raw API shape, top-level { "players": [...] }).
#
...
echo "Fetching FFC PPR ADP for: $SEASONS"
...
for s in $SEASONS; do
  ...
  if [ "${FFC_FORCE:-0}" != "1" ] && [ "$s" -lt "$DYNAMIC_FROM" ] && [ -s "$dest" ] \
    ...
    echo "  [skip] $s — already have $(player_count "$dest") players"
    kept=$((kept + 1))
    continue
  fi

  url="https://fantasyfootballcalculator.com/api/v1/adp/ppr?teams=12&year=${s}"
  tmp="$(mktemp)"
  if curl -fsSL -A "$UA" -H 'Accept: application/json' "$url" -o "$tmp"; then
    n="$(player_count "$tmp")"
    if [ "$n" -gt 0 ]; then
      mv "$tmp" "$dest"
      echo "  [ok]   $s — saved $n players"
      fetched=$((fetched + 1))
    else
      echo "  [warn] $s — response had 0 players; keeping existing file"
      rm -f "$tmp"
      failed=$((failed + 1))
    fi
  else
    echo "  [warn] $s — fetch failed; keeping existing file"
    rm -f "$tmp"
    failed=$((failed + 1))
  fi
done
