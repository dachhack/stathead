...
| `load_weekly_projections()` | Per-week matchup-adjusted projections incl. kickers + team DST, one row per player-game (opp, home, matchup mult, PPR pts, gsis/sleeper ids, depth-chart rank, roster `status`/`active` — IR/exempt/practice-squad rows are zeroed from the current week with the conditional number in `proj_ppr_if_active` — and a `backup` flag for 1–3 game lines; `df.attrs['def_vs_pos']`, `df.attrs['meta']['currentWeek']`) | ~780 × 17 rows/player |
